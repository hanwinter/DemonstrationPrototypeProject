...
const defaultVisibleStageKeys = stages.slice(0, 9).map((stage) => stage.key)
...
const visitTableWidth = computed(() => (88 + visibleStages.value.length * 114) + 'px')
...
<template>
  <section class="visit-card task-card">
    <div class="section-head"><h2>产后回访提醒</h2><el-tooltip content="根据客户生产日期计算当前产后阶段，展示当前需要执行的产后回访任务。" placement="top"><button class="hint-btn" type="button" aria-label="产后回访提醒说明"><el-icon><InfoFilled /></el-icon></button></el-tooltip></div>
    <div class="filter-row"><div class="filter-left"><label><span>顾问筛选</span><el-select v-model="advisorFilter" filterable size="small" class="tool-select"><el-option v-for="item in advisorOptions" :key="item" :label="item" :value="item" /></el-select></label><span class="advisor-total">顾问总数：{{ advisorTotal }}人</span></div><div class="table-meta"><button class="expand-btn" type="button" @click="openStageSettings">设置</button></div></div>
    <div v-if="!showAllAdvisors" class="visit-table-scroll">
      <el-table :data="displayedRows" border :style="{ width: visitTableWidth, minWidth: visitTableWidth }" class="visit-table excel-table" :header-cell-class-name="visitHeaderCellClassName" @header-click="handleVisitHeaderClick">
        ...
        <el-table-column align="center" header-align="center">
          ...
          <el-table-column v-for="stage in visibleStages" :key="stage.key" :prop="stage.key" :label="stage.headerLabel" width="114" align="center" header-align="center">
    ...
    <div v-else class="visit-table-scroll">
      <el-table :data="displayedRows" border :style="{ width: visitTableWidth, minWidth: visitTableWidth }" class="visit-table excel-table is-expanded" :header-cell-class-name="visitHeaderCellClassName" @header-click="handleVisitHeaderClick">
        <el-table-column prop="consultantName" label="负责顾问" fixed width="88" align="center" header-align="center" class-name="advisor-column" header-class-name="advisor-header" />
        <el-table-column align="center" header-align="center">
          <template #header><span class="group-header">待回访产妇数量<el-tooltip content="产后阶段按照左闭右开规则计算：15≤产后天数＜30，30≤产后天数＜45，依次类推。" placement="top"><button class="hint-btn table-hint" type="button" aria-label="产后阶段计算规则"><el-icon><InfoFilled /></el-icon></button></el-tooltip></span></template>
          <el-table-column v-for="stage in visibleStages" :key="stage.key" :prop="stage.key" :label="stage.headerLabel" width="114" align="center" header-align="center">
  ...
  <section class="visit-card risk-card">
    <div class="sub-head overdue-head"><h3>超期分析</h3><button class="overdue-summary" type="button" @click="emit('open-detail', { mode: 'overdue', visitType: '产后回访', consultantName: overdueAdvisorFilter === '全部' ? '' : overdueAdvisorFilter, stage: overdueStageFilterKeys.length === overdueStageOptions.length ? '' : overdueStageFilterKeys.join(',') })"><span>超期未回访产妇：</span><strong>{{ filteredOverdueTotal }}</strong><em>人</em></button></div>
