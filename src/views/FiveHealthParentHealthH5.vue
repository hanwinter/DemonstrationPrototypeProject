...
const aiAnalysisIcon = new URL('../../ProjectImage/AI分析.png', import.meta.url).href
const trainingFeedbackIcon = new URL('../../ProjectImage/训练反馈.png', import.meta.url).href
const doctorConsultIcon = new URL('../../ProjectImage/咨询医生.png', import.meta.url).href
const addPhotoIcon = new URL('../../ProjectImage/添加照片.png', import.meta.url).href
...
const followupExamPlans = [
  { age: '春季五健筛查', date: '2026-03-20', items: '完成身高体重、视力、口腔、脊柱、心理行为基础筛查。', status: '已完成' },
  { age: '筛查报告生成', date: '2026-03-28', items: '生成五健筛查报告，提示视力下降、远视储备不足等需关注项。', status: '已完成' },
  { age: '视力专项复查预约', date: '2026-04-05', items: '家长确认复查时间，预约眼科专项检查。', status: '已完成' },
  { age: '当前复诊', date: '2026-04-18', items: '进行裸眼视力、屈光、眼轴、远视储备等复诊检查，并生成专科建议。', status: '当前' },
  { age: '家庭干预计划', date: '2026-04-25', items: '推送户外记录、用眼记录、远眺训练、眼保健操等家庭干预任务。', status: '待完成' },
  { age: '一个月随访', date: '2026-05-18', items: '跟踪用眼习惯、户外时长、训练执行情况和眼部不适反馈。', status: '待随访' },
  { age: '三个月复查', date: '2026-07-18', items: '复查视力、屈光、眼轴变化，评估近视进展风险。', status: '待安排' },
  { age: '秋季五健复测', date: '2026-09-20', items: '结合秋季筛查，复测视力、BMI、脊柱体态等重点项目。', status: '待安排' },
]
...
const projectSubPageTitle = computed(() => ({ profile: '儿童建档', questionnaire: '首诊问卷', consent: '专案知情同意书', specialtyReport: '专科诊疗报告', followup: '复诊计划', parentManual: '图文报告', onlineInteraction: 'AI+医生在线问诊', followSurvey: '随访问卷', familyTrainingRecord: '家庭训练记录' }[activeProjectSubPage.value] || '专案详情'))
const mergedTopTitle = computed(() => {
  if (page.value === 'projectSubPage') return projectSubPageTitle.value
  if (page.value === 'heightWeightTest') return '身高体重自测'
  if (page.value === 'nutritionSurvey') return '营养调查'
  return ''
})
const isMergedTopPage = computed(() => Boolean(mergedTopTitle.value))
...
<template>
  <main class="parent-demo">
    <section :class="['phone-shell', { 'is-login-shell': !isLoggedIn }]">
      <header :class="['phone-top', { 'is-subpage-top': isMergedTopPage }]">
        <template v-if="isMergedTopPage">
          <button class="back" type="button" @click="page === 'projectSubPage' ? backToProjectFlow() : backFromAiTestSubPage()"><el-icon><ArrowLeft /></el-icon></button>
          <strong>{{ mergedTopTitle }}</strong>
      ...
      <div :class="['phone-content', { 'login-content': !isLoggedIn, 'sheet-open': showHealthDetailSheet }]">
        ...
        <section v-else-if="page === 'projectSubPage'" class="screen project-subpage-screen">
          <article v-if="activeProjectSubPage !== 'onlineInteraction'" class="project-sub-summary">
            <strong>{{ currentStudent.name }}｜{{ currentStudent.gender }}｜{{ currentStudent.age }}岁</strong>
          ...
          <template v-else-if="activeProjectSubPage === 'consent'">
            ...
            <section v-if="consentSigned" class="sub-section signed-info consent-signed-detail"><div class="signed-info-head"><h3>签署信息</h3><span>已完成签署</span></div><p><em>签署人</em><b>林一凡家长</b></p><p><em>签署时间</em><b>{{ consentSignTime }}</b></p><div class="signed-signature"><em>签名图片</em><img :src="consentSignatureData" alt="家长手写签名" /></div></section>
          </template>
          ...
          <template v-else-if="activeProjectSubPage === 'onlineInteraction'">
            <section class="ai-consult-status"><div><strong>{{ currentStudent.name }}｜{{ currentProject.name }}</strong><p>AI已初步分析 · 医生已回复</p></div></section>
            <section class="interaction-flow ai-consult-flow">
              <article class="parent consult-card"><p>孩子完成家庭视觉训练后说眼睛酸，看远处黑板不太清楚，想咨询是否需要调整训练。</p><small>04月16日 20:10</small></article>
              <article class="image-note consult-upload"><span>家庭训练打卡照片 1张</span><small>04月16日 20:12</small></article>
              <article class="ai-assistant consult-card"><strong>AI助手初步分析</strong><div class="ai-analysis-grid"><p><em>可能原因</em><b>训练后短暂视疲劳、近距离用眼时间偏长，需结合持续时间判断。</b></p><p><em>风险提示</em><b>当前信息偏低风险，但看远不清楚需要医生结合复查数据确认。</b></p><p><em>建议处理</em><b>先暂停加量训练，记录症状变化，并等待医生确认建议。</b></p></div><div class="consult-tags"><span>低风险</span><span>需医生确认</span></div><small>04月16日 20:13</small></article>
              <article class="doctor consult-card"><strong>眼科医生 王医生</strong><p>医生已查看 AI 初步分析和家长反馈。建议本周训练强度保持原计划，不额外加量；每次训练后远眺 5-10 分钟。如连续3天仍明显眼酸或看远不清，请预约复诊。</p><div class="consult-tags"><span>医生已确认</span></div><small>04月17日 09:30</small></article>
              <article class="parent consult-card"><p>收到，会按建议记录训练后的眼酸情况，若持续不缓解再预约复诊。</p><small>04月17日 10:05</small></article>
            </section>
            <section class="interaction-input ai-consult-input"><div class="consult-compose-bar"><textarea placeholder="请输入"></textarea><button class="consult-send-entry" type="button" @click="sendFamilyMessage">发送</button></div><div class="consult-more-panel"><button type="button"><img :src="aiAnalysisIcon" alt="" /><span>AI分析</span></button><button type="button"><img :src="trainingFeedbackIcon" alt="" /><span>训练反馈</span></button><button type="button"><img :src="doctorConsultIcon" alt="" /><span>咨询医生</span></button><button type="button" @click="uploadFamilyImage"><img :src="addPhotoIcon" alt="" /><span>添加照片</span></button></div></section>
          </template>
...
.phone-shell,.phone-shell.is-login-shell{border:1px solid #d8e6df!important;border-radius:0!important;box-shadow:0 12px 28px rgba(32,52,58,.10)!important}
.parent-demo{background:#0b0b0b!important}
...
@media (hover:none) and (pointer:coarse), (max-width:768px){
  .parent-demo{width:100vw!important;min-height:100dvh!important;height:100dvh!important;padding:0!important;margin:0!important;background:#0b0b0b!important;overflow:hidden!important}
...
.consent-signed-detail{display:flex!important;flex-direction:column!important;gap:8px!important;padding:12px 13px!important;background:#fff!important}.signed-info-head{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:10px!important}.signed-info-head span{flex:none!important;padding:3px 8px!important;border-radius:999px!important;background:#E8F8F1!important;color:#18B884!important;font-size:12px!important;font-weight:800!important;white-space:nowrap!important}
...
.phone-shell:has(.ai-consult-input) .phone-content{overflow:hidden!important;padding-bottom:14px!important}.project-subpage-screen:has(.ai-consult-input){position:relative!important;height:100%!important;min-height:0!important;padding-bottom:150px!important;overflow:hidden!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-flow{min-height:0!important;overflow-y:auto!important;scrollbar-width:thin!important;scrollbar-color:rgba(96,117,124,.24) transparent!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-flow::-webkit-scrollbar{width:4px!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-flow::-webkit-scrollbar-track{background:transparent!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-flow::-webkit-scrollbar-thumb{background:rgba(96,117,124,.22)!important;border-radius:999px!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-input{position:absolute!important;left:0!important;right:0!important;bottom:0!important;z-index:5!important}.project-subpage-screen:has(.ai-consult-input) .consult-more-panel img{width:30px!important;height:30px!important;display:block!important;object-fit:contain!important}.project-subpage-screen:has(.ai-consult-input) .consult-more-panel button{height:72px!important}.project-subpage-screen:has(.ai-consult-input) .consult-more-panel span{margin-top:0!important}.signature-overlay{background:#000!important}.signature-device{border:1px solid #2f2f2f!important;border-radius:0!important;box-shadow:none!important;background:#fff!important}.signature-device-speaker{background:#2f2f2f!important;box-shadow:none!important}.signature-screen{border-radius:0!important;border:1px solid #2f2f2f!important}.signature-actions-bar button{border:0!important;border-radius:8px!important;background:#F4F8F7!important;color:#60757C!important;box-shadow:none!important}.signature-actions-bar button:last-child{background:#12A8AD!important;color:#fff!important}.signature-actions-bar button:disabled{opacity:.55!important}.signed-signature{display:flex!important;flex-direction:column!important;gap:7px!important}
...
.phone-shell:has(.project-subpage-screen) .bottom-tabs{display:none!important}.phone-shell:has(.project-subpage-screen) .phone-content{height:calc(100% - 54px)!important;padding-bottom:112px!important}.phone-shell:has(.ai-consult-input) .phone-content{height:calc(100% - 54px)!important;padding:14px 14px 0!important;overflow:hidden!important}.phone-shell:has(.ai-consult-input) .bottom-tabs{display:none!important}
...
.phone-top.is-subpage-top{display:grid!important;grid-template-columns:34px minmax(0,1fr) 34px!important;gap:8px!important;align-items:center!important;justify-content:normal!important}.phone-top.is-subpage-top strong{min-width:0!important;text-align:left!important;color:#20343A!important;font-size:18px!important;font-weight:800!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.phone-top.is-subpage-top .back{border-radius:0!important;background:transparent!important;color:#20343A!important;box-shadow:none!important}.ai-consult-status{padding:13px 14px;border:0;border-radius:8px;background:#fff;box-shadow:0 8px 22px rgba(28,91,92,.055);display:flex;align-items:center;justify-content:space-between;gap:10px}.ai-consult-status strong{color:#20343A;font-size:15px}.ai-consult-status p{margin-top:4px;color:#60757C;font-size:12px}.ai-consult-status span,.consult-tags span{padding:3px 8px;border-radius:999px;background:#E8F8F1;color:#18B884;font-size:12px;font-weight:800;white-space:nowrap}.ai-consult-flow .consult-card{display:flex;flex-direction:column;gap:6px}.interaction-flow article.ai-assistant{align-self:flex-start;background:#F7FBFA;border-color:rgba(216,238,234,.9)}.ai-analysis-grid{display:flex;flex-direction:column;gap:7px}.ai-analysis-grid p{margin:0!important;padding:8px;border-radius:8px;background:#fff!important;border:1px solid rgba(216,238,234,.62)}.ai-analysis-grid em{display:block;color:#8A9CA1;font-size:11px;font-style:normal;font-weight:800}.ai-analysis-grid b{display:block;margin-top:3px;color:#20343A;font-size:13px;line-height:1.45}.consult-tags{display:flex;gap:6px;flex-wrap:wrap}.consult-upload{display:flex!important;flex-direction:column!important;gap:5px}.quick-consult-actions{margin:0 0 8px!important;display:flex!important;justify-content:flex-start!important;gap:7px!important;flex-wrap:wrap!important}.quick-consult-actions button{height:26px!important;padding:0 9px 0 7px!important;border:0!important;border-radius:999px!important;background:#F4F8F7!important;color:#60757C!important;font-size:12px!important;font-weight:700!important;display:inline-flex!important;align-items:center!important;gap:4px!important;box-shadow:none!important}.quick-consult-actions button i{width:15px!important;height:15px!important;border-radius:50%!important;display:inline-grid!important;place-items:center!important;background:#E4F8F6!important;color:#12A8AD!important;font-size:10px!important;font-style:normal!important;font-weight:800!important;line-height:1!important}.ai-consult-input textarea{min-height:58px!important}.ai-test-screen .route-return-title{justify-content:center}.ai-student-card,.ai-test-entry,.ai-form-card,.ai-result-card,.ai-info-card,.nutrition-question-card{border:0;border-radius:8px;background:#fff;box-shadow:0 8px 22px rgba(28,91,92,.055)}.ai-student-card{padding:14px;display:flex;align-items:center;justify-content:space-between;gap:10px}.ai-student-card strong{color:#20343A;font-size:16px}.ai-student-card p{margin-top:5px;color:#60757C;font-size:13px}.ai-student-card span,.ai-result-head span{padding:3px 8px;border-radius:999px;background:#E8F8F1;color:#18B884;font-size:12px;font-weight:800;white-space:nowrap}.ai-test-entry{width:100%;min-height:58px;padding:14px;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;border:0;text-align:left}.ai-test-entry strong{display:block;color:#20343A;font-size:15px}.ai-test-entry p{margin-top:5px;color:#60757C;font-size:13px;line-height:1.45}.ai-test-entry em{color:#12A8AD;font-size:13px;font-style:normal;font-weight:800;white-space:nowrap}.ai-test-note{padding:0 4px;color:#8A9CA1;font-size:12px;line-height:1.55}.ai-form-card,.ai-result-card,.nutrition-question-card{padding:14px}.ai-form-card{display:flex;flex-direction:column;gap:12px}.ai-form-card h3,.nutrition-question-card h3{margin:0;color:#20343A;font-size:15px}.ai-form-card label{display:flex;flex-direction:column;gap:7px;color:#60757C;font-size:13px}.ai-input-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.unit-input{display:grid;grid-template-columns:minmax(0,1fr) 42px;align-items:center;border:1px solid rgba(216,238,234,.9);border-radius:8px;background:#FAFEFD;overflow:hidden}.unit-input input{border:0!important;background:transparent!important;box-shadow:none!important}.unit-input b{color:#12A8AD;font-size:13px;text-align:center}.ai-result-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px}.ai-result-head strong{color:#20343A;font-size:16px}.ai-result-head span.warning{background:#FFF4E8;color:#F2994A}.ai-result-text{display:flex;flex-direction:column;gap:7px;color:#60757C;font-size:13px;line-height:1.6}.ai-result-text p{margin:0;color:#60757C;background:transparent;padding:0;border-radius:0}.ai-result-text b{display:inline;color:#20343A;font-size:13px;line-height:1.6}.ai-result-text h3{margin:5px 0 0;color:#20343A;font-size:14px;line-height:1.4}.ai-info-card{padding:13px 14px}.ai-info-card p{color:#60757C;font-size:13px;line-height:1.55}.nutrition-question-card article{padding:10px 0;border-top:1px solid rgba(216,238,234,.62)}.nutrition-question-card article:first-of-type{border-top:0}.nutrition-question-card h4{margin:0 0 8px;color:#20343A;font-size:14px;line-height:1.45}.nutrition-question-card .primary{margin-top:10px}@media(max-width:360px){.ai-test-entry{grid-template-columns:1fr}.ai-test-entry em{justify-self:start}.ai-input-grid{grid-template-columns:1fr}}
.ai-consult-flow{background:#F7F8FA!important;box-shadow:none!important;border:0!important;padding:12px 10px!important;gap:10px!important}.ai-consult-flow article{position:relative!important;max-width:82%!important;border:0!important;box-shadow:none!important;padding:8px 10px!important;border-radius:8px!important}.ai-consult-flow article.parent{align-self:flex-end!important;background:#A8F0A2!important;color:#111827!important;border-top-right-radius:3px!important}.ai-consult-flow article.ai-assistant,.ai-consult-flow article.doctor{align-self:flex-start!important;background:#F2F3F5!important;color:#1F2937!important;border-top-left-radius:3px!important}.ai-consult-flow article.parent strong,.ai-consult-flow article.parent small{color:rgba(17,24,39,.68)!important;text-align:right!important}.ai-consult-flow article.ai-assistant strong,.ai-consult-flow article.doctor strong{color:#7B8794!important;font-size:12px!important;font-weight:700!important}.ai-consult-flow article p{color:inherit!important;margin:2px 0!important;line-height:1.55!important}.ai-consult-flow article small{color:#8A9CA1!important;font-size:11px!important}.ai-consult-flow .consult-upload{align-self:flex-end!important;background:#A8F0A2!important;color:#111827!important;border-top-right-radius:3px!important}.ai-consult-flow .consult-upload span{color:#111827!important;font-weight:700!important}.ai-analysis-grid{gap:5px!important}.ai-analysis-grid p{padding:0!important;border:0!important;border-radius:0!important;background:transparent!important}.ai-analysis-grid em{color:#6B7280!important;font-size:12px!important}.ai-analysis-grid b{color:#1F2937!important;font-size:13px!important}.ai-consult-flow .consult-tags{display:none!important}.ai-consult-input{background:#F6F7F8!important;box-shadow:none!important;border:0!important;padding:10px!important;gap:10px!important}.consult-compose-bar{display:grid!important;grid-template-columns:minmax(0,1fr) 52px!important;gap:8px!important;align-items:center!important}.consult-compose-bar textarea{width:100%!important;height:40px!important;min-height:40px!important;max-height:40px!important;padding:9px 11px!important;background:#fff!important;border:1px solid #E5E7EB!important;border-radius:11px!important;box-shadow:none!important;color:#1F2937!important;line-height:20px!important;resize:none!important;overflow:hidden!important}.consult-compose-bar textarea::placeholder{color:#94A3B8!important}.consult-send-entry{height:40px!important;padding:0 12px!important;border:0!important;border-radius:11px!important;background:#12A8AD!important;color:#fff!important;box-shadow:none!important;font-size:13px!important;font-weight:800!important;white-space:nowrap!important}.consult-more-panel{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:8px!important}.consult-more-panel button{min-width:0!important;height:70px!important;padding:8px 4px!important;border:0!important;border-radius:12px!important;background:#fff!important;box-shadow:none!important;color:#374151!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;gap:6px!important}.consult-more-panel i{width:28px!important;height:28px!important;border-radius:10px!important;background:#E8F8F6!important;color:#12A8AD!important;font-style:normal!important;font-size:13px!important;font-weight:800!important;display:grid!important;place-items:center!important;line-height:1!important}.consult-more-panel span{font-size:12px!important;font-weight:700!important;line-height:1.2!important;white-space:nowrap!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-flow{flex:1 1 auto!important;-webkit-overflow-scrolling:touch!important}.project-subpage-screen:has(.ai-consult-input) .ai-consult-status{flex:none!important}.specialty-summary-card,.specialty-detail-section{padding:14px;border:0;border-radius:8px;background:#fff;box-shadow:0 8px 22px rgba(28,91,92,.055)}.specialty-summary-card{display:grid;grid-template-columns:1fr 1fr;gap:8px}.specialty-summary-card>div{grid-column:1/-1;display:flex;align-items:center;justify-content:space-between;gap:10px}.specialty-summary-card strong{color:#20343A;font-size:16px}.specialty-summary-card span,.specialty-tags span{padding:4px 8px;border-radius:999px;background:#FFF4E8;color:#C76D12;font-size:12px;font-weight:800}.specialty-summary-card p,.specialty-note-grid p{min-width:0;padding:8px;border-radius:8px;background:#F7FBFA}.specialty-summary-card em,.specialty-note-grid em,.specialty-plan-list em{display:block;color:#8A9CA1;font-size:11px;font-style:normal;line-height:1.35}.specialty-summary-card b,.specialty-note-grid b,.specialty-plan-list b{display:block;margin-top:4px;color:#20343A;font-size:13px;line-height:1.45}.specialty-detail-section{padding:0;overflow:hidden}.specialty-section-toggle{width:100%;min-height:42px;padding:0 14px;border:0;background:#fff;display:flex;align-items:center;justify-content:space-between;gap:10px}.specialty-section-toggle span{display:flex;align-items:center;gap:7px;color:#20343A;font-size:15px;font-weight:800}.specialty-section-toggle em{font-style:normal;color:#12A8AD;font-size:12px;font-weight:800}.specialty-section-body{padding:0 14px 14px;border-top:1px solid rgba(216,238,234,.62)}.specialty-eye-table{margin-top:12px;border:1px solid rgba(216,238,234,.72);border-radius:8px;overflow:hidden}.specialty-eye-table .head,.specialty-eye-table p{display:grid;grid-template-columns:72px minmax(0,1fr) minmax(0,1fr);gap:0}.specialty-eye-table .head{background:#F0FCFA;color:#60757C;font-size:12px;font-weight:800}.specialty-eye-table span,.specialty-eye-table b{padding:8px 7px;border-top:1px solid rgba(216,238,234,.62);font-size:12px;line-height:1.35;text-align:left}.specialty-eye-table .head span{border-top:0}.specialty-eye-table b{color:#20343A;font-weight:700}.specialty-note-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.specialty-tags{display:flex;flex-wrap:wrap;gap:7px;margin-top:12px}.specialty-paragraph{margin-top:10px;color:#20343A;font-size:13px;line-height:1.65}.specialty-plan-list{display:flex;flex-direction:column;gap:10px;margin-top:12px}.specialty-plan-list p{padding:10px;border-radius:8px;background:#F7FBFA}.specialty-attachment-list{display:flex;flex-direction:column;margin-top:12px}.specialty-attachment-list button{height:42px;border:0;border-top:1px solid rgba(216,238,234,.62);background:#fff;display:flex;align-items:center;justify-content:space-between;color:#20343A;font-size:13px;font-weight:700}.specialty-attachment-list button:first-child{border-top:0}.specialty-attachment-list span{color:#12A8AD;font-size:12px;font-weight:800}
