    ...
    <section :class="['phone-shell', { 'is-login-shell': !isLoggedIn }]">
      ...
      <div :class="['phone-content', { 'login-content': !isLoggedIn, 'sheet-open': showHealthDetailSheet }]">
        ...
        <section v-else-if="page === 'aiTest'" class="screen ai-test-screen">
          ...
          <article class="ai-student-card"><div><strong>{{ currentStudent.name }}｜{{ currentStudent.gender }}｜{{ currentStudent.age }}岁</strong><p>当前儿童</p></div><span>AI自测</span></article>
          <button class="ai-test-entry" type="button" @click="go('heightWeightTest')"><span><strong>身高体重自测</strong><p>输入性别、出生年月、身高、体重，生成生长发育评价结果</p></span><em>开始自测 &gt;</em></button>
          <button class="ai-test-entry" type="button" @click="go('nutritionSurvey')"><span><strong>营养调查</strong><p>填写营养问卷，生成饮食与营养风险评价</p></span><em>开始自测 &gt;</em></button>
        ...
        <section v-else-if="page === 'heightWeightTest'" class="screen ai-test-sub-screen">
          ...
          <article class="ai-student-card"><div><strong>{{ currentStudent.name }}｜{{ currentStudent.gender }}｜{{ currentStudent.age }}岁</strong><p>AI自测</p></div><span>生长发育</span></article>
          ...
          <section v-if="heightWeightResultVisible" class="ai-result-card"><div class="ai-result-head"><strong>评价结果</strong><span>正常</span></div><div class="ai-result-grid"><p><em>BMI</em><b>19.3</b></p><p><em>身高评价</em><b>正常</b></p><p><em>体重评价</em><b>正常</b></p></div><p><em>生长发育评价</em><b>处于同年龄段正常范围</b></p><p><em>建议</em><b>继续保持规律饮食、充足睡眠和适量运动</b></p></section>
        ...
        <section v-else-if="page === 'nutritionSurvey'" class="screen ai-test-sub-screen">
          ...
          <article class="ai-student-card"><div><strong>{{ currentStudent.name }}｜{{ currentStudent.gender }}｜{{ currentStudent.age }}岁</strong><p>AI自测</p></div><span>营养风险</span></article>
          ...
          <section v-if="nutritionResultVisible" class="ai-result-card nutrition-result-card"><div class="ai-result-head"><strong>营养评价结果</strong><span class="warning">中风险</span></div><div class="ai-problem-list"><strong>主要问题</strong><p>蔬菜水果摄入不足</p><p>奶类摄入不足</p><p>含糖饮料摄入偏多</p></div><div class="ai-problem-list"><strong>改善建议</strong><p>每日保证蔬菜水果摄入</p><p>每日补充奶类或奶制品</p><p>减少含糖饮料和高糖零食</p><p>保持规律早餐和充足睡眠</p></div></section>
        </section>
