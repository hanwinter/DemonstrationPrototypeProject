      ...
      <section class="phone-content">
        ...
        <section v-else-if="activeTab === 'project'" class="screen project-screen">
          ...
          <template v-else-if="activeProjectPanel === 'report'">
            ...
            <template v-else-if="activeReportDoc">
              <section class="report-doc-page"><div class="doc-title"><button type="button" @click="closeReportDoc">返回</button><div><strong>图文报告</strong><p>{{ activeReportDoc.name }}｜{{ activeReportDoc.date }}</p></div></div><img :src="activeReportDoc.image" :alt="activeReportDoc.name" /></section>
            </template>
            <template v-else>
